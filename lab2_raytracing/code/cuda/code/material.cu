...
__device__ bool lambertian::scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState *local_rand_state) const {
    /**
     * @brief represents ideal matte surface using "Lambertian reflectance"
     * @note https://en.wikipedia.org/wiki/Lambertian_reflectance
     */
...
__device__ bool dielectric::scatter(const ray& r_in,
                                const hit_record& rec,
                                vec3& attenuation,
                                ray& scattered,
                                curandState *local_rand_state) const  {
    /**
     * @brief represents materials like water, glass and etc.
     */
    vec3 outward_normal;
